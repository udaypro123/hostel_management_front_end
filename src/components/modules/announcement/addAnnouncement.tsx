...
const AddAnnouncement: React.FC = () => {
    ...
    const { isEdit, announcedata } = useLocation().state || { isEdit: false };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit && announcedata) {
                announment._id = announcedata._id;
            }
            if (isEdit && announcedata) {
                console.log("announment 3", announment)
                await updateAnnouncement(announment);
                setSuccess('Announment updated successfully!');
                enqueueSnackbar('Announment updated successfully!', { variant: 'success' });
                navigate('/announcements'); // Redirect to rooms page after update
            } else {
                console.log("announment 2", announment)
                await createAnnouncement(announment);
                setSuccess('Announment added successfully!');
                enqueueSnackbar('Announment added successfully!', { variant: 'success' });
                navigate('/announcements'); // Redirect to rooms page after addition
            }
            // Reset form
            setFormData({
                hostel: '',
    ...
    useEffect(() => {
        if (isEdit && announcedata) {
            setFormData({
                document:announcedata?.document[0],
                hostel: announcedata?.hostelId || '',
                    ...
                    <Box sx={{ width: "96%" }}>
                        <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', width: '100%', }}>
                            ...
                            <Button
                                ...
                                sx={{ minWidth: 120 }}
                            >
                                {isLoading ? 'Adding...' : isEdit ? "Update Announcement" : 'Add Announcement'}
                            </Button>
                        </Box>
                    </Box>
                </Grid>
